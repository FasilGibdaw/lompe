...
QUIVERSCALES = {'ground_mag':       600 * 1e-9,  # ground magnetic field scale [T]
                ...
                # electric surface current density [A/m] Ohm's law
                'electric_current': 1000 * 1e-3,
                # electric surface current density [A/m] SECS
                'secs_current':     1000 * 1e-3,
...
COLORSCALES = {'fac':        np.linspace(-1.95, 1.95, 40) * 1e-6 * 2,
               # upward component
               'ground_mag': np.linspace(-980, 980, 50) * 1e-9 / 3,
...
def plot_coastlines(ax, model, resolution='110m', **kwargs):
    ...
    for cl in model.grid_J.projection.get_projected_coastlines(resolution=resolution):
        xi, eta = cl
        if model.dipole:
            lon, lat = model.grid_J.projection.cube2geo(
                xi, eta)  # retrieve geographic coords
            mlat, mlon = cd.geo2mag(lat, lon)  # convert to magnetic
            xi, eta = model.grid_J.projection.geo2cube(
                mlon, mlat)  # and back to xi, eta
...
def plot_mlt(ax, model, time, apex, mltlevels=np.r_[0:24:3], txtkwargs=None, **kwargs):
    ...
    default_txtkwargs = {'bbox': {'facecolor': 'white', 'alpha': .5,
                                  'edgecolor': 'none'}, 'ha': 'center', 'va': 'center'}
    if txtkwargs == None:
    ...
    for mltlevel in mltlevels:
        ...
        if np.sum(iii) > 2:
            xi, eta = model.grid_J.projection.geo2cube(
                lon[iii], lat[iii])  # to xi, eta
            ax.plot(xi, eta, **kwargs)
            ax.text(xi[len(xi)//2], eta[len(xi)//2],
                    str(np.int32(mltlevel)).zfill(2), **txtkwargs)
...
def format_ax(ax, model, apex=None, **kwargs):
    ...
    ax.format_coord = lambda xi, eta: 'lon = {:.2f}, lat = {:.2f}'.format(
        * tuple(projection.cube2geo(xi, eta)))
...
def plot_quiver(ax, model, dtype, scale=None, **kwargs):
    ...
    xi, eta = np.meshgrid(np.linspace(
        ximin, ximax, sh[1]), np.linspace(etamin, etamax, sh[1]))
    ...
    x, y, Ax, Ay = model.grid_J.projection.vector_cube_projection(
        Ae, An, lo, la)
...
def plot_contour(ax, model, dtype, vertical=False, **kwargs):
    ...
    if z.size == model.grid_J.xi.size:
        xi, eta, z = model.grid_J.xi, model.grid_J.eta, z.reshape(
            model.grid_J.shape)
    else:
        xi, eta, z = model.grid_E.xi, model.grid_E.eta, z.reshape(
            model.grid_E.shape)
...
def polarplot(ax, model, apex, time, dV=None, **clkw):
    ...
    grid = model.grid_E
    xs = (grid.lon_mesh[0, :], grid.lon_mesh[-1, :],
          grid.lon_mesh[:, 0], grid.lon_mesh[:, -1])
    ys = (grid.lat_mesh[0, :], grid.lat_mesh[-1, :],
          grid.lat_mesh[:, 0], grid.lat_mesh[:, -1])
    for i, c in enumerate(zip(xs, ys)):
        lon, lat = c
        if not model.dipole:
            lat, lon = apex.geo2apex(
                lat, lon, (model.R-RE)*1e-3)   # to magnetic apex
        mlt = cd.mlon2mlt(lon, time)
        pax.plot(lat, mlt, color='black',
                 linewidth=1.5 if i == 0 else .5, zorder=2)
    ...
    if dV != None:  # plot electric potential
        ...
        pax.contour(lat, mlt, V, levels=levels,
                    colors='C0', linewidths=1, zorder=3)
...
def plot_SECS_amplitudes(ax, model, curl_free=True, **kwargs):
    ...
    kwargs['norm'] = plt.matplotlib.colors.BoundaryNorm(
        levels, ncolors=kwargs['cmap'].N, clip=True)
...
def lompeplot(model, figheight=9, include_data=False, show_data_location=False, apex=None, time=None,
    ...
    arrowax.text(5, 1, '{:.0f} nT (ground), {:.0f} nT (space)\n{:.0f} mA/m, {:.0f} m/s'.format(
        quiverscales['ground_mag'] * 1e9 // 2, quiverscales['space_mag_full'] * 1e9 // 2, quiverscales['electric_current'] * 1e3 // 2, quiverscales['convection'] // 2), ha='left', va='top')
    ...
    buax.set_xlim(colorscales['ground_mag'].min() *
                  1e9, colorscales['ground_mag'].max() * 1e9)
    buax.set_xlabel('nT')

    conductance_levels = colorscales['hall']
    xx = np.vstack((conductance_levels, conductance_levels))
    yy = np.vstack((np.zeros_like(conductance_levels),
                   np.ones_like(conductance_levels)))
    ...
    # Finish
    # ------
    plt.subplots_adjust(top=0.91, bottom=0.065, left=0.01,
                        right=0.99, hspace=0.1, wspace=0.02)
    ...
    if isinstance(includeData, dict):
        ...
        for dtype, dcol in zip(dtypes, colors):
            if len(model.data[dtype]) == 0:
                continue
            for dataset in model.data[dtype]:
                xi_d, eta_d = model.grid_E.projection.geo2cube(
                    dataset.coords['lon'], dataset.coords['lat'])
...
def resolutionplot(model, apex=None, savekw=None, return_axes=False,
    ...
    var = np.ma.array(model.etaRes, mask=(
        model.xiResFlag+model.etaResFlag) < 2)
...
def locerrorplot(model, apex=None, savekw=None, return_axes=False,
    ...
    var = np.ma.array(model.resL, mask=(
        model.xiResFlag + model.etaResFlag) < 2)
...
def Cdplot(model, dtype, apex=None, savekw=None, return_axes=False,
    ...
    if 'mag' in dtype:
        ...
        if dtype == 'ground_mag':
            coords = {'lon': grid.lon.flatten(), 'lat': grid.lat.flatten(),
                      'r': np.ones(grid.size)*6371.2e3}
        else:
            coords = {'lon': grid.lon.flatten(), 'lat': grid.lat.flatten(),
                      'r': np.ones(grid.size)*model.R}
    ...
    for ax, G, comp in zip(axes, Gs, components):
        ...
        Cdpost = np.sqrt(np.diag(G.dot(model.Cmpost).dot(G.T))
                         ).reshape(grid.shape)
        Cdpost *= manScale
        Cdpost *= scale
        ...
        # Add title
        if not dtype == 'fac':
            ax.text(0.5, 1.02, comp, ha='center', va='bottom',
...
def model_data_scatterplot(model, fig_parameters={'figsize': (8, 8)},
    ...
    for dtype in model.data.keys():  # loop through data types
        for ds in model.data[dtype]:  # loop through the datasets within each data type
            ...
            ds = ds.subset(model.biggrid.ingrid(
                ds.coords['lon'], ds.coords['lat'], ext_factor=perimiter_width))
            ...
            if 'mag' in dtype:
                ...
                for B, d, sym in zip(Bs, ds.values, ['>', '^', 'o']):
                    ax.scatter(d/scaling, B/scaling, marker=sym,
                               c='C' + str(counter), alpha=.7)

            if (dtype in ['convection', 'efield']):
                Ge, Gn = model.matrix_func[dtype](**ds.coords)

                if ds.los is not None:  # deal with line of sight data:
                    G = Ge * ds.los[0].reshape((-1, 1)) + \
                        Gn * ds.los[1].reshape((-1, 1))
                    ax.scatter(ds.values/scaling, G.dot(model.m)/scaling,
                               c='C' + str(counter), marker='x', zorder=4, alpha=.7)
                if ds.los is None:
                    Es = [Ge.dot(model.m), Gn.dot(model.m)]
                    for E, d, sym in zip(Es, ds.values, ['>', '^']):
                        ax.scatter(d/scaling, E/scaling, marker=sym,
                                   c='C' + str(counter), zorder=4, alpha=.7)
    ...
    ax.plot([extent*.9, extent*.9], [-extent + .1, -
            extent + .1 + nranges], 'k-', zorder=7)
    # scale
    ax.plot([extent*.89, extent*.91], [-extent + .1]*2, 'k-', zorder=7)
    ax.plot([extent*.89, extent*.91],
            [-extent + .1 + nranges]*2, 'k-', zorder=7)
    ...
    for dtype in model.data.keys():  # loop through data types
        for ds in model.data[dtype]:  # loop through the datasets within each data type
            ...
            if 'mag' in dtype:
                ax.text(extent * (.9 - counter * .05), -extent + .1 + nranges/2, str(int((scaling *
                        nranges * 1e9))) + ' nT', c='C' + str(counter), va='center', ha='right', rotation=90)

            if dtype == 'convection':
                ax.text(extent * (.9 - counter * .05), -extent + .1 + nranges/2, str(int((scaling *
                        nranges))) + ' m/s', c='C' + str(counter), va='center', ha='right', rotation=90)

            if dtype == 'efield':
                ax.text(extent * (.9 - counter * .05), -extent + .1 + nranges/2,
                        str(int((scaling * nranges * 1e3))) + ' V/m', c='C' + str(counter))

            ax.text(-extent + .1, extent - .3 - counter * .25, ds.label.replace('_',
                    ' '), color='C' + str(counter), va='top', ha='left', size=14)
    ...
    ax.scatter(extent+1, extent+1, marker='x',
               c='black', label='line-of-sight')
