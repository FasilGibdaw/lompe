...
class Data(object):
    def __init__(self, values, coordinates=None, LOS=None, LOS_mag=None, components='all', datatype='none', label=None, scale=None, iweight=None, error=0):
        """
        ...
        errors = {'ground_mag': 10e-9, 'space_mag_full': 30e-9,
                  'space_mag_fac': 30e-9, 'convection': 50, 'efield': 3e-3, 'fac': 1e-6}
        iweights = {'ground_mag': 0.5, 'space_mag_full': 0.5,
                    'space_mag_fac': 0.5, 'convection': 1.0, 'efield': 1.0, 'fac': 1.0}
        ...
        if np.array(error).size == 1:
            if error == 0:
                ...
                warnings.warn(
                    f"'error' keyword not set for datatype '{datatype}'! Using error={error}", UserWarning)

        if iweight is None:
            iweight = iweights[datatype]
            warnings.warn(
                f"'iweight' keyword not set for datatype '{datatype}'! Using iweight={iweight}", UserWarning)
        ...
        if coordinates is not None:
            if datatype.lower() == 'fac':
                warnings.warn(
                    'Warning: FAC data must be defined on the whole Emodel.grid_J, but this is not checked.', UserWarning)
            if coordinates.shape[0] == 2:
                self.coords = {'lon': coordinates[0], 'lat': coordinates[1]}
            if coordinates.shape[0] == 3:
                self.coords = {
                    'lon': coordinates[0], 'lat': coordinates[1], 'r': coordinates[2]}
        else:
            self.coords = {}
            assert datatype.lower(
            ) == 'fac', "coordinates must be provided for all datatypes that are not 'fac'"
        self.isvalid = True
        if np.ndim(self.values) == 2:
            ...
            raise ValueError(
                f'Too many dimensions within data values ndim: {np.ndim(values)} cannot exceed 2.\nData array must be of shape (number of data points, vectors)\ne.g. 3D magnetometer measurement can have shape (100, 3) where ndim=2')

        if (LOS is not None) & (datatype in ['convection', 'efield']):
            # should be (2, N) east, north components of line of sight vectors
            self.los = LOS
            # 2021-10-29: jreistad added this to work with how components is used in model.py. Could avoid this slightly non-inututuve value by modifying model.py instead.
            self.components = [0, 1]
        else:
            self.los = None
            if type(components) == str and components == 'all':
                ...
                assert np.all([i in [0, 1, 2] for i in self.components]
                              ), 'component(s) must be in [0, 1, 2]'

        if (LOS_mag is not None) & (datatype in ['convection', 'efield']):
            # should be (2, N) east, north components of line of sight vectors
            self.los_mag = LOS_mag
            # D Billett adding magnetic vectors in
            self.los_mag_components = [0, 1]
        else:
            self.los_mag = None
            self.los_mag_components = None
        ...
        if self.coords['lat'].size != np.array(self.values, ndmin=2).shape[1]:
            raise ShapeError(
                'not the same number of coordinates and data points')
    ...
    def subset(self, indices):
        ...
        if self.los_mag is not None:
            self.los_mag = self.los_mag[:, indices]
